...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { project_id, user_email = "", captionStyle = "pill" } = payload;
    ...
    const agent = await anthropic.beta.agents.create({
      ...
      system: buildVideoSystemPrompt(project_id, aiGenId, captions, supabaseUrl, supabaseServiceKey, supabaseAnonKey, klingApiKey, captionStyle, user_email),
...
Example: Caption "Google's AI competes with Claude" → Scene: "Two glowing holographic interfaces facing each other in a dark server room, screens filled with abstract blurred data streams and soft light, cinematic vertical 9:16, photorealistic, no legible text, no readable characters, any on-screen text must be blurred or stylized"

Save these as SCENE_1 … SCENE_5.

---

## PHASE 1 — Generate voiceover AND submit kie.ai tasks (run these 6 HTTP calls in parallel, then STOP and wait for all responses)
...
### 1b–1f. Submit 5 kie.ai video tasks (one per caption, all at the same time)
Use SCENE_1 through SCENE_5 (from Phase 0) as the prompt for each task — NOT the raw caption text.

POST https://api.kie.ai/api/v1/jobs/createTask
Headers: Authorization: Bearer ${klingApiKey} / Content-Type: application/json
Body for each (replace SCENE_N with the scene description you wrote):
{
  "model": "kling-3.0/video",
  "input": { "aspect_ratio": "9:16", "duration": "5", "sound": false, "mode": "std", "multi_shots": false, "prompt": "SCENE_N" }
}
→ Save the taskId from each response as TASK_ID_1 … TASK_ID_5.

After all 6 responses arrive, write a Supabase checkpoint:
PATCH ${supabaseUrl}/rest/v1/ai_generations?project_id=eq.${projectId}
Body: { "kling_task_ids": ["TASK_ID_1","TASK_ID_2","TASK_ID_3","TASK_ID_4","TASK_ID_5"], "status": "kling_tasks_created" }

If createTask returned an error for any task, write the error text into debug_log and set status to "kling_create_error". Then STOP — do not proceed further.

---

## PHASE 2 — Poll kie.ai until all 5 clips are ready (DO NOT proceed to Phase 3 until this is done)

For each TASK_ID, poll every 15 seconds:
...
When state is anything other than "waiting", "generating", or "completed" (e.g. "failed"), record null for that video URL and continue with the rest.

After all 5 polls finish, write a Supabase checkpoint:
PATCH ${supabaseUrl}/rest/v1/ai_generations?project_id=eq.${projectId}
...
## ABSOLUTE RULES
1. Execute phases in order: 1 → 2 → 3 → 4. Never skip a phase.
